...
class EventsOriginRepository:
    ...
    def list_events(self, events_threshold: int, limit: int) -> List[EventObj]:
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                    ...
                    WHERE stg.bonussystem_events.id>  coalesce((SELECT 
	                                        max(workflow_settings->>'last_loaded_id')::int as last_loaded_id
                                        FROM stg.srv_wf_settings AS sws
	                                        where workflow_key = 'example_events_origin_to_stg_workflow'),0) ;
                """,
                {
                    "id": event.id,
